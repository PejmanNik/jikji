...
function SectionLayoutInitializer({children}: SectionLayoutInitializerProps) {
  ...
  useEffect(() => {
    if (!isLayoutSuspended && pageRef.current != null) {
      ...
      const pages = splitPageContent(pageRef.current, pageAdjustments, plugins);

      setPages(pages);
    }
  }, [isLayoutSuspended, plugins]);

  const isSplitted = pages !== undefined;
  return (
    <>
      <PrerenderPreview isSplitted={isSplitted}>
        <Page
          pageNumber={0}
          totalPages={0}
          ref={pageRef}
      ...
      {isSplitted && (
        <SectionLayoutBuilder sectionChildren={sectionChildren} pages={pages} />
      )}
    </>
  );
}
